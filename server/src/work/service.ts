...
import { activityListSchema, productsSchema, type ProductInput } from './schemas.js';
...
interface ListedActivityRow extends ActivityRow {
  ...
  specifications: string; ready: boolean;
}
...
interface ProductRow {
  ...
  length: string | null; width: string | null; specifications: string;
}
...
function lineCents(quantity: number, unitValue: number): number {
  const mills = Math.round(quantity * 1000);
  const unitCents = Math.round(unitValue * 100);
  const raw = mills * unitCents;
...
export async function createOrderProducts(tx: SqlConnection, orderId: string, products: ProductInput[]): Promise<void> {
  const parsed = productsSchema.parse(products);
  const parent = (await tx.query<{ value: string }>('SELECT value FROM orders WHERE id = $1 FOR UPDATE', [orderId])).rows[0];
  ...
  const assigned = [...new Set(parsed.flatMap(product => product.activities.map(activity => activity.assignedUserId).filter((id): id is string => !!id)))];
  ...
  for (const [index, product] of parsed.entries()) {
    ...
    await tx.query(`
      INSERT INTO order_products (id, order_id, position, description, quantity, unit_value, length, width, specifications)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
    `, [productId, orderId, index + 1, product.description, product.quantity, product.unitValue,
      product.length ?? null, product.width ?? null, product.specifications]);
    ...
    for (const [position, activity] of product.activities.entries()) {
      await tx.query(`
        INSERT INTO order_activities (id,order_id,product_id,position,area,assigned_user_id)
        VALUES ($1,$2,$3,$4,$5,$6)
      `, [randomUUID(), orderId, productId, position + 1, activity.area, activity.assignedUserId ?? null]);
    }
  }
}
...
function activityDto(row: ListedActivityRow, materials: MaterialRow[]) {
  return {
    ...
    productDescription: row.product_description, specifications: row.specifications,
...
export async function workOrder(db: Database, auth: AuthSession, orderId: string) {
  return db.transaction(async tx => {
    ...
    const products = (await tx.query<ProductRow>(
      'SELECT id,order_id,position,description,quantity,unit_value,line_total,length,width,specifications FROM order_products WHERE order_id=$1 ORDER BY position',
      [orderId],
    )).rows;
    ...
    return {
      ...
      products: visible.map(product => ({
        ...
        quantity: Number(product.quantity), specifications: product.specifications,
        ...(product.length !== null && product.width !== null
          ? { length: Number(product.length), width: Number(product.width) } : {}),
...
export async function changeActivity(
  ...
  return db.transaction(async tx => {
    ...
    if (action === 'claim') {
      ...
      if (!roleForArea[current.area].includes(actor.role)) throw new ApiError(403, 'FORBIDDEN', 'Tu perfil no opera esta actividad.');
      if (current.area === 'DESIGN' && current.assigned_user_id !== actor.id) throw conflict('Debes tomar o recibir esta tarea antes de trabajarla.');
        ...
        if (current.status === 'IN_PROGRESS') {
          ...
          if (Number(pending.rows[0].count) === 0) {
            ...
            await tx.query(`
              ...
              WHERE id=$1
            `, [current.order_id, next, stamp, parent.requires_installation]);
            await tx.query(`
