...
const workIcons: Record<WorkStatus, LucideIcon> = { NEW: FileText, PENDING_ADMIN_REVIEW: ClipboardCheck, IN_PRINTING: Printer, IN_WORKSHOP: Hammer, PENDING_INSTALLATION: Hammer, COMPLETED: Check, INSTALLED: Check };
export function WorkBadge({ status }: { status: WorkStatus }) { const Icon = workIcons[status]; const tone = ['NEW','PENDING_ADMIN_REVIEW'].includes(status) ? 'amber' : status === 'IN_PRINTING' ? 'red' : ['IN_WORKSHOP','PENDING_INSTALLATION'].includes(status) ? 'blue' : 'green'; return <span className={`badge badge-${tone}`}><Icon size={13} aria-hidden="true" />{STATUS_LABELS[status]}</span>; }
export function PaymentBadge({ status }: { status: PaymentStatus }) { const Icon = status === 'PAID' ? Check : status === 'PARTIAL' ? Wallet : CircleDashed; return <span className={`badge badge-${status === 'PAID' ? 'green' : 'amber'}`}><Icon size={13} aria-hidden="true" />{status === 'PAID' ? 'Pagada' : status === 'PARTIAL' ? 'Abono' : 'Pendiente'}</span>; }
