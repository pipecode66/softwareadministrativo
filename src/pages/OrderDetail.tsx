...
function ProductionSteps({ order }: { order: WorkOrder }) {
  const steps = [
    ...
    ...(!['WORKSHOP_ONLY'].includes(order.route) ? [{ id: 'printing', label: 'Impresión', icon: Printer, detail: order.printingCompletedAt ? formatDate(order.printingCompletedAt) : order.status === 'IN_PRINTING' ? 'Etapa actual' : 'Siguiente etapa', complete: !!order.printingCompletedAt, active: order.status === 'IN_PRINTING' }] : []),
