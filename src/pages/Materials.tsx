...
import { areaOf, currentMonthRange, dateOnly, formatDate, formatMeasure, formatNumber, inRange, MATERIALS } from '../domain/utils';
...
export function MaterialsPage() {
  ...
  return <div className="page-stack analytics-page">
    ...
    <Card className="analytics-material-banner"><div><span className="analytics-banner-icon"><Ruler size={26} /></span><div><span className="eyebrow">SUPERFICIE IMPRESA DEL PERÍODO</span><strong>{formatMeasure(total)} <small>m²</small></strong><span className="muted">{rows.length} órdenes con impresión finalizada</span></div></div><div className="analytics-material-strip"><div>{materials.map(row => <span key={row.name} title={`${row.name}: ${formatMeasure(row.value)} m²`} style={{ background: row.color, width: `${total ? row.value / total * 100 : 0}%` }} />)}</div><span>Distribución entre los materiales del catálogo</span></div></Card>
    <div className="analytics-material-grid">{materials.map(row => <div key={row.name} className="analytics-material-kpi" style={{ borderTopColor: row.color }}><KpiCard label={row.name} value={<>{formatMeasure(row.value)} <small>m²</small></>} icon={Layers} tone="slate" meta={`${row.count} órdenes en la selección`} /></div>)}</div>
    <div className="analytics-material-charts"><Card><CardHeader title="Distribución de superficie" description="Participación por material · metros cuadrados" />{total > 0 ? <><div className="analytics-donut analytics-material-donut" aria-label="Distribución de superficie impresa por material"><ResponsiveContainer width="100%" height="100%"><PieChart accessibilityLayer><Pie isAnimationActive={false} data={materials.filter(row => row.value > 0)} dataKey="value" nameKey="name" innerRadius="66%" outerRadius="89%" paddingAngle={2} stroke="none">{materials.filter(row => row.value > 0).map(row => <Cell key={row.name} fill={row.color} />)}</Pie><Tooltip formatter={value => `${formatMeasure(Number(value))} m²`} /></PieChart></ResponsiveContainer><div className="analytics-donut-center"><span>Total impreso</span><strong>{formatMeasure(total)}</strong><small>m²</small></div></div><ul className="analytics-breakdown">{materials.map(row => <li key={row.name}><span><i style={{ background: row.color }} />{row.name}</span><div><strong>{formatMeasure(row.value)} m²</strong><small>{formatNumber(row.value / total * 100, 1)} %</small></div></li>)}</ul></> : <EmptyState title="Sin consumo registrado" description="Solo se contabilizan las órdenes cuya impresión ya finalizó." />}</Card>
    <Card><CardHeader title="Evolución de la superficie impresa" description={`${monthly ? 'Agrupación mensual' : 'Agrupación diaria'} · fecha de finalización de impresión`} />{rows.length ? <><div className="analytics-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={timeline} margin={{ top: 10, right: 6, bottom: 8, left: 0 }} accessibilityLayer><CartesianGrid vertical={false} stroke="#e2e8f0" strokeDasharray="3 5" /><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#64748b' }} minTickGap={28} /><YAxis tickFormatter={value => formatMeasure(value)} width={50} axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#64748b' }} /><Tooltip formatter={value => `${formatMeasure(Number(value))} m²`} /><Bar isAnimationActive={false} dataKey="area" name="Superficie impresa" fill="#f97316" radius={[4, 4, 0, 0]} maxBarSize={32} /></BarChart></ResponsiveContainer></div><details className="analytics-chart-data"><summary>Consultar metros cuadrados por período</summary><div className="analytics-table-scroll"><table className="table"><caption className="sr-only">Superficie impresa por período</caption><thead><tr><th>Período</th><th>Superficie</th></tr></thead><tbody>{timeline.map(row => <tr key={row.key}><th scope="row">{row.label}</th><td>{formatMeasure(row.area)} m²</td></tr>)}</tbody></table></div></details></> : <EmptyState title="Sin impresiones finalizadas en estas fechas" description="Cambia el período o el material para consultar otros resultados." />}<div className="analytics-card-bottom"><span>Promedio por orden impresa</span><strong>{formatMeasure(rows.length ? total / rows.length : 0)} m²</strong></div></Card></div>
