...
import { ArrowLeft, ArrowRight, ArrowUpRight, Building2, CheckCircle2, ClipboardList, Pencil, Phone, Plus, Users, Wallet } from 'lucide-react';
import { useApp } from '../data/AppContext';
import { apiClientOrderHistory } from '../data/api';
...
function ClientEditor({ open, client, onClose, onSaved }: { open: boolean; client?: Client; onClose: () => void; onSaved: (client: Client) => void }) {
  ...
  const [specialPayment, setSpecialPayment] = useState(client?.specialPayment ?? false);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try {
      const result = await saveClient({ name: name.trim(), identification: identification.trim(), phone: phone.trim(), specialPayment }, client?.id);
      onSaved(result);
    } catch (cause) {
  ...
  return <Modal open={open} onClose={onClose} title={client ? 'Editar cliente' : 'Nuevo cliente'}>
    <form onSubmit={submit} className="stack">
      ...
      <Field label="Nombre o razón social *" htmlFor="client-name">
        <input id="client-name" className="input" autoComplete="organization" value={name} onChange={event => setName(event.target.value)} maxLength={180} required autoFocus />
      </Field>
      <Field label="NIT o identificación" htmlFor="client-identification" hint="Opcional para REM; necesario para FACT.">
        <input id="client-identification" className="input" value={identification} onChange={event => setIdentification(event.target.value)} maxLength={60} />
      </Field>
      <Field label="Celular *" htmlFor="client-phone">
        <input id="client-phone" className="input" type="tel" autoComplete="tel" value={phone} onChange={event => setPhone(event.target.value)} maxLength={40} required />
      </Field>
      <label className="row wrap"><input type="checkbox" checked={specialPayment} onChange={event => setSpecialPayment(event.target.checked)} /><span>Cliente Especial: permite crear una OT sin abono inicial.</span></label>
...
export function ClientsPage() {
  ...
  const rows = data.clients.map(client => {
    ...
    return { ...client, orderCount: orders.length, balance: admin ? orders.reduce((total, order) => total + financials(order).balance, 0) : 0, activeCount: orders.filter(order => !['COMPLETED', 'INSTALLED'].includes(order.status)).length };
  });
  ...
  return <div className="page-stack">
    ...
    <Card>
      <div className="toolbar ops-toolbar">
        ...
        <label className="ops-inline-field"><span className="ops-sr-only">Filtrar clientes</span><select className="select" value={filter} onChange={event => { setFilter(event.target.value); setPage(1); }}><option value="all">Todos los clientes</option><option value="active">Con trabajos en curso</option>{admin && <option value="balance">Con saldo pendiente</option>}</select></label>
      </div>
      {!filtered.length ? <EmptyState title={search || filter !== 'all' ? 'No encontramos clientes' : 'Tu directorio está listo'} description={search || filter !== 'all' ? 'Prueba otro nombre o cambia el filtro.' : 'Registra el primer cliente para asociar sus órdenes.'} action={<Button variant="secondary" onClick={() => search || filter !== 'all' ? (setSearch(''), setFilter('all'), setPage(1)) : setEditor('new')}>{search || filter !== 'all' ? 'Limpiar filtros' : 'Crear cliente'}</Button>} /> : <>
        <DataTable columns={[
          { key: 'name', label: 'Cliente', render: row => <div className="ops-client-cell"><span className="ops-avatar" aria-hidden="true">{initials(row.name)}</span><div><Link className="cell-title link" to={`/clients/${row.id}`}>{row.name}</Link><span className="cell-subtitle">{row.identification || 'Sin identificación'}{row.specialPayment ? ' · Especial' : ''}</span></div></div> },
          ...
          ...(admin ? [{ key: 'orders', label: 'Órdenes', render: (row: (typeof rows)[number]) => <span className="ops-tabular">{row.orderCount}</span> }] : []),
          ...(admin ? [{ key: 'balance', label: 'Saldo pendiente', render: (row: (typeof rows)[number]) => <span className={`money ${row.balance > 0 ? 'ops-amount-pending' : ''}`}>{formatCOP(row.balance)}</span> }] : []),
...
export function ClientDetailPage() {
  const { id } = useParams();
  const { data, user, toast, usingApi } = useApp();
  const [editing, setEditing] = useState(false);
  const [filter, setFilter] = useState('all');
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);
  const [remoteOrders, setRemoteOrders] = useState<WorkOrder[] | null>(null);
  const [historyLoading, setHistoryLoading] = useState(false);
  const [historyError, setHistoryError] = useState('');
  const client = data.clients.find(item => item.id === id);
  useEffect(() => {
    if (!usingApi || !id) { setRemoteOrders(null); setHistoryLoading(false); setHistoryError(''); return; }
    let active = true;
    setRemoteOrders(null); setHistoryLoading(true); setHistoryError('');
    void (async () => {
      const items: WorkOrder[] = [];
      let nextPage = 1;
      let hasMore = true;
      while (hasMore) {
        const result = await apiClientOrderHistory(id, nextPage, 100);
        items.push(...result.items);
        hasMore = result.hasMore && result.items.length > 0;
        nextPage += 1;
      }
      if (active) setRemoteOrders([...new Map(items.map(order => [order.id, order])).values()]);
    })().catch(cause => {
      if (active) setHistoryError(cause instanceof Error ? cause.message : 'No fue posible consultar el historial del cliente.');
  ...
  const orders = remoteOrders ?? data.orders.filter(order => order.clientId === client.id);
  const balance = admin ? orders.reduce((sum, order) => sum + financials(order).balance, 0) : 0;
  const paid = admin ? orders.reduce((sum, order) => sum + financials(order).paid, 0) : 0;
  const filtered = orders.filter(order => normalize(`OT #${String(order.number).padStart(4, '0')} ${order.number} ${order.description}`).includes(normalize(search.trim())) && (filter === 'all' || (filter === 'balance' ? admin && financials(order).balance > 0 : ['COMPLETED', 'INSTALLED'].includes(order.status)))).sort((a, b) => b.number - a.number);
  const safePage = Math.min(page, Math.max(1, Math.ceil(filtered.length / 6)));
  const statusCell = (order: WorkOrder) => <div className="ops-status-stack"><WorkBadge status={order.status} />{admin && <PaymentBadge status={financials(order).paymentStatus} />}</div>;
  const canOpenOrder = (order: WorkOrder) => admin || (usingApi
    ? data.orders.some(item => item.id === order.id)
    : order.createdBy === user.id || Boolean(order.products?.some(product => product.activities.some(activity => activity.area === 'DESIGN' && (!activity.assignedUserId || activity.assignedUserId === user.id)))));
  const orderNumber = (order: WorkOrder) => canOpenOrder(order)
    ? <Link className="cell-title link" to={`/orders/${order.id}`}>OT #{String(order.number).padStart(4, '0')}</Link>
    : <span className="cell-title">OT #{String(order.number).padStart(4, '0')}</span>;

  return <div className="page-stack">
    <Link className="ops-back-link" to="/clients"><ArrowLeft size={16} /> Directorio de clientes</Link>
    <PageHeader eyebrow="Ficha de cliente" title={client.name} description={`NIT / Identificación: ${client.identification || 'Sin registrar'}${client.specialPayment ? ' · Cliente Especial' : ''}`} actions={<Button variant="secondary" onClick={() => setEditing(true)}><Pencil size={16} /> Editar cliente</Button>} />
    {admin && <div className="grid-3">
    ...
    <Card>
      <CardHeader title="Órdenes de trabajo" description={admin ? 'Consulta el estado operativo y de pago de cada trabajo.' : 'Consulta el estado operativo de cada trabajo.'} />
      {historyLoading && <p className="muted" aria-live="polite">Cargando el historial completo del cliente…</p>}
      {historyError && <p className="notice notice-warning" role="alert">{historyError}</p>}
      <div className="toolbar ops-toolbar"><SearchInput value={search} onChange={value => { setSearch(value); setPage(1); }} placeholder="Buscar OT o descripción…" label="Buscar órdenes del cliente" /><label className="ops-inline-field"><span className="ops-sr-only">Filtrar órdenes del cliente</span><select className="select" value={filter} onChange={event => { setFilter(event.target.value); setPage(1); }}><option value="all">Todas las órdenes</option>{admin && <option value="balance">Con saldo pendiente</option>}<option value="completed">Terminadas o instaladas</option></select></label></div>
      {!filtered.length ? <EmptyState title="Sin órdenes para mostrar" description={orders.length ? 'Prueba otra búsqueda o filtro.' : 'Las órdenes de este cliente aparecerán aquí.'} /> : <><DataTable columns={[
        { key: 'number', label: 'Orden', render: orderNumber },
        ...
        ...(admin ? [{ key: 'balance', label: 'Saldo', render: (order: WorkOrder) => <span className="money">{formatCOP(financials(order).balance)}</span> }] : []),
      ]} rows={filtered.slice((safePage - 1) * 6, safePage * 6)} rowKey={order => order.id} renderCard={order => <div className="ops-mobile-record"><div className="ops-record-footer">{orderNumber(order)}<DocumentBadge type={order.documentType} /></div><p>{order.description}</p>{statusCell(order)}<div className="ops-mobile-record-stats"><span>{formatDate(order.createdAt)}</span>{admin && <span className="money">{formatCOP(financials(order).balance)} por cobrar</span>}</div></div>} />{admin ? <Pagination page={safePage} pageSize={6} total={filtered.length} onChange={setPage} /> : <div className="pagination"><p>Página <strong>{safePage}</strong></p><nav aria-label="Paginación del historial"><button className="icon-button" type="button" aria-label="Página anterior" disabled={safePage <= 1} onClick={() => setPage(safePage - 1)}><ArrowLeft size={16} /></button><button className="icon-button" type="button" aria-label="Página siguiente" disabled={safePage * 6 >= filtered.length} onClick={() => setPage(safePage + 1)}><ArrowRight size={16} /></button></nav></div>}</>}
    </Card>
