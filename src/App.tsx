...
export default function App(){return <AppErrorBoundary><AppProvider><BrowserRouter><Suspense fallback={<Loading/>}><Routes><Route path="/login" element={<LoginPage/>}/><Route path="/change-password" element={<Guard allowPasswordChange><ChangePasswordPage/></Guard>}/><Route element={<Guard><AppShell/></Guard>}><Route index element={<Home/>}/><Route path="operation" element={<Guard roles={operationsAdmins}><OperationPage/></Guard>}/><Route path="orders" element={<Guard roles={[...operationsAdmins,'DISENO','IMPRESION','TALLER']}><OrdersPage/></Guard>}/><Route path="orders/new" element={<Guard roles={[...operationsAdmins,'DISENO']}><OrderFormPage/></Guard>}/><Route path="orders/:id/edit" element={<Guard roles={operationsAdmins}><OrderFormPage/></Guard>}/><Route path="orders/:id/print" element={<Guard roles={operationsAdmins}><PrintOrderPage/></Guard>}/><Route path="orders/:id" element={<Guard roles={[...operationsAdmins,'DISENO','IMPRESION','TALLER']}><OrderDetailPage/></Guard>}/><Route path="clients" element={<Guard roles={[...operationsAdmins,'DISENO']}><ClientsPage/></Guard>}/><Route path="clients/:id" element={<Guard roles={[...operationsAdmins,'DISENO']}><ClientDetailPage/></Guard>}/><Route path="portfolio" element={<Guard roles={operationsAdmins}><PortfolioPage/></Guard>}/><Route path="reports" element={<Guard roles={admins}><ReportsPage/></Guard>}/><Route path="materials" element={<Guard roles={operationsAdmins}><MaterialsPage/></Guard>}/><Route path="design" element={<Guard roles={[...operationsAdmins,'DISENO']}><QueuePage department="DISENO"/></Guard>}/><Route path="printing" element={<Guard roles={[...operationsAdmins,'IMPRESION']}><QueuePage department="IMPRESION"/></Guard>}/><Route path="workshop" element={<Guard roles={[...operationsAdmins,'TALLER']}><QueuePage department="TALLER"/></Guard>}/><Route path="settings/users" element={<Guard roles={['ADMINMASTER']}><UsersPage/></Guard>}/><Route path="*" element={<EmptyState title="Esta página no existe" description="Comprueba la dirección o vuelve al inicio." action={<Link className="btn btn-primary" to="/">Ir al inicio</Link>}/>}/></Route></Routes></Suspense></BrowserRouter></AppProvider></AppErrorBoundary>;}
